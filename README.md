...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/gazette_search_notices \
  -H 'Content-Type: application/json' \
  -d '{"query":"Tesco","category":"corporate insolvency"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/gazette_search_notices`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "uk-gazette": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-uk-gazette"]
    }
  }
}
```
